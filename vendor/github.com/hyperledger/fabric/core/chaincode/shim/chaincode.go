...
func (stub *ChaincodeStub) GetKeyForKey(key string) ([]byte, error) {
	response, err := stub.handler.handleGetKeyForKey(key, stub.ChannelId, stub.TxID)
	if err != nil {
		return nil, err
	}
	return response,nil
...
func (iter *KeyQueryIterator) Next() (*queryresult.KeyModification, error) {
	fmt.Println("next1")
	if result, err := iter.nextResult(Key_QUERY_RESULT); err == nil {
		fmt.Println("next2")
		return result.(*queryresult.KeyModification), err
	} else {
		fmt.Println("next3")
