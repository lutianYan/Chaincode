...
func (handler *Handler) handleGetKeyForKey(key string, channelId string, txid string) ([]byte, error) {
	// Create the channel on which to communicate the response from validating peer
	var respChan chan pb.ChaincodeMessage
	var err error
	if respChan, err = handler.createChannel(channelId, txid); err != nil {
	...
	// Send GET_HISTORY_FOR_KEY message to peer chaincode support
	//we constructed a valid object. No need to check for error
	payloadBytes, _ := proto.Marshal(&pb.GetKeyForKey{Key: key})

	msg := &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_GET_KEY_FOR_KEY, Payload: payloadBytes, Txid: txid, ChannelId: channelId}
	chaincodeLogger.Debugf("[%s] Sending %s", shorttxid(msg.Txid), pb.ChaincodeMessage_GET_KEY_FOR_KEY)

	var responseMsg pb.ChaincodeMessage

	if responseMsg, err = handler.sendReceive(msg, respChan); err != nil {
		chaincodeLogger.Errorf("[%s] error sending %s", shorttxid(msg.Txid), pb.ChaincodeMessage_GET_KEY_FOR_KEY)
		return nil, errors.Errorf("[%s] error sending %s", shorttxid(msg.Txid), pb.ChaincodeMessage_GET_KEY_FOR_KEY)
	}
	...
	if responseMsg.Type.String() == pb.ChaincodeMessage_RESPONSE.String() {
		...
		//chaincodeLogger.Debugf("[%s] Received %s. Successfully got range", shorttxid(responseMsg.Txid), pb.ChaincodeMessage_RESPONSE)
		//
		//getKeyForKeyResponse := &pb.QueryResponse{}
		//if err = proto.Unmarshal(responseMsg.Payload, getKeyForKeyResponse); err != nil {
		//	chaincodeLogger.Errorf("[%s] unmarshall error", shorttxid(responseMsg.Txid))
		//	return nil, errors.Errorf("[%s] unmarshal error", shorttxid(responseMsg.Txid))
		//}
